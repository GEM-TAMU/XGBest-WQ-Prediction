...
accuracy_ML$Model <- "XGB_biased"
summary_ML$Model <- "XGB_biased"
...
accuracy_ML_bias_corrected$Model <- "XGB"
summary_ML_bias_corrected$Model <- "XGB"

accuracy_WRTDS <- check_accuracy(WRTDS_cv_outputs)
...
summary_table <- summary_table%>%
  filter(Model %in% c("LOADEST", "WRTDS","XGB"))%>%
  select(Par,Model,test_NSE_gt_0.3, test_KGE_gt_0.3,test_pbias_15)
